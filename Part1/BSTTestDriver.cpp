...
int main() {
  cout << endl << "TESTING BST" << endl;
  BST newTree;
  WordPair a("apple", "knsgsf");
  WordPair b("jobs", "vsIaf'af");
  WordPair c("quality", "ag'plemg");
  WordPair d("attribute", "mogalWds");
  WordPair e("bow", "Ojsfc");
  
  newTree.insert(a);
  newTree.insert(e);
  newTree.insert(c);
  cout << "Printing current tree: " << endl; 
  newTree.traverseInOrder(visit); 
  cout << endl;

  cout << "Retrieve first word pair: ";
  cout << newTree.retrieve(a);
  newTree.insert(d);
  cout << "Current element count after inserting 1 WordPair: " << newTree.getElementCount() << endl;

  try {
    cout << "Retrieve word pair not in BST: " << endl;
    cout << newTree.retrieve(b);
  }
